...
{
	return (rel(elements[pos1].info.first, elements[pos2].info.first) || elements[pos1].info.first == elements[pos2].info.first);
}
...
{
	int current_node = this->root_idx;
	while (current_node != -1)
		...
		else if (this->rel(c, this->elements[current_node].info.first))
	...
	current_node = this->root_idx;
	while (current_node != -1)
	{
		if (this->rel(elements[position].info.first, elements[current_node].info.first))
		{
...
TValue SortedMap::search(TKey c) const
{
	...
	while (current_node != -1)
	{
		...
		if (this->rel(c, this->elements[current_node].info.first))
			current_node = elements[current_node].left;
		else
			current_node = elements[current_node].right;
	}
...
TValue SortedMap::remove(TKey c)
{
	...
	{
		parent = current_node;
		if (this->rel(c, this->elements[current_node].info.first) || c == elements[current_node].info.first)
			current_node = elements[current_node].left;
		else
			current_node = elements[current_node].right;
	}
	...
	if (elements[current_node].left == -1)
	{
		if (parent == -1)
		{
			root_idx = elements[current_node].right;
	...
	else if (elements[current_node].right == -1)
	{
		if (parent == -1)
		{
			root_idx = elements[current_node].left;
			TValue old_value = this->elements[current_node].info.second;
			free_node(current_node);
			return old_value;
		}
